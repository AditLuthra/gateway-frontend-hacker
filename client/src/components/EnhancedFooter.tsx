  ...
  return (
    <footer className="bg-dark-bg-secondary border-t border-makr-blue/30 text-dark-text-primary transition-colors relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-16 pb-12 relative z-10">
          ...
          <div className="lg:col-span-4">
            ...
            <div className="space-y-3 text-sm text-dark-text-muted font-mono">
              <div className="flex items-center gap-3">
                <MapPin className="w-4 h-4 text-makr-yellow flex-shrink-0" />
                <span>Bangalore, Mumbai, Delhi, Pune & 25+ cities across India</span>
              </div>
              <div className="flex items-center gap-3">
                <Phone className="w-4 h-4 text-makr-yellow flex-shrink-0" />
                <a href="tel:+918047258000" className="hover:text-makr-blue transition-colors">
                  +91 80472 58000
                </a>
              </div>
              <div className="flex items-center gap-3">
                <Mail className="w-4 h-4 text-makr-yellow flex-shrink-0" />
                <a href="mailto:hello@makrx.org" className="hover:text-makr-blue transition-colors">
                  hello@makrx.org
                </a>
              </div>
              <div className="flex items-center gap-3">
                <Globe className="w-4 h-4 text-makr-yellow flex-shrink-0" />
                  ...
                  <ul className="space-y-3">
                    {section.links.map((link) => (
                      <li key={link.name}>
                        <Link
                          to={link.href}
                          className="group flex flex-col text-dark-text-muted hover:text-makr-blue transition-colors"
                        >
                          <span className="font-medium font-mono group-hover:text-makr-yellow transition-colors">
                            {link.name}
                          </span>
                          <span className="text-xs text-dark-text-muted mt-1 font-mono">
                            {link.description}
                          </span>
                        </Link>
                      </li>
          ...
          <div className="max-w-2xl mx-auto text-center">
            ...
            <form onSubmit={handleNewsletterSubmit} className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto">
              ...
              <input
                ...
                className="flex-1 px-4 py-3 rounded-lg bg-dark-surface border border-makr-blue/30 text-dark-text-primary placeholder-dark-text-muted focus:border-makr-yellow focus:outline-none focus:shadow-neon font-mono"
                aria-required="true"
              ...
              <Button
                type="submit"
                className="btn-cyberpunk px-6 py-3 font-mono font-semibold"
            ...
            <p id="newsletter-description" className="text-xs text-dark-text-muted mt-3 font-mono">
              ...
              <Link to="/privacy" className="text-makr-yellow hover:text-makr-blue transition-colors">
                Privacy Policy
              </Link>
            </p>
          </div>
                ...
                {socialLinks.map((social) => {
                  ...
                  return (
                    <a
                      ...
                      className={`p-2 rounded-lg bg-dark-surface border border-makr-blue/30 text-makr-blue ${social.color} transition-all hover:scale-110 hover:shadow-neon`}
            ...
            <div className="flex flex-wrap items-center gap-6 text-sm">
              {legalLinks.map((link, index) => (
                <React.Fragment key={link.name}>
                  <Link
                    to={link.href}
                    className="text-dark-text-muted hover:text-makr-blue transition-colors font-mono"
                  >
                    {link.name}
                  </Link>
                  {index < legalLinks.length - 1 && (
                    <span className="text-makr-blue">|</span>
                  )}
                </React.Fragment>
              ))}
            </div>
        ...
        <div className="border-t border-makr-blue/30 pt-8 mt-8">
          <div className="flex flex-col sm:flex-row items-center justify-center gap-8 text-xs text-dark-text-muted font-mono">
            ...
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-makr-blue rounded-full animate-pulse"></div>
              <span>GDPR Compliant</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="w-3 h-3 bg-makr-yellow rounded-full animate-pulse"></div>
      ...
      <Button
        ...
        className="fixed bottom-6 right-6 p-3 btn-cyberpunk rounded-full shadow-neon hover:shadow-neon-lg transition-all hover:scale-110 z-40"
