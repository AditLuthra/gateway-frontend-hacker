...
export default function Docs() {
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      case 'guide': return 'bg-makr-blue/20 text-makr-blue border border-makr-blue/30';
      case 'api': return 'bg-purple-500/20 text-purple-400 border border-purple-500/30';
      case 'video': return 'bg-red-500/20 text-red-400 border border-red-500/30';
      case 'example': return 'bg-terminal-green/20 text-terminal-green border border-terminal-green/30';
  ...
  return (
    <>
      <Helmet>
        <title>MakrX Documentation - Guides, APIs & Tutorials</title>
        <meta 
          name="description" 
          content="Comprehensive MakrX documentation with guides, API references, and tutorials. Learn makerspace management, integrations, and platform features." 
        />
      </Helmet>
      ...
      <div className="min-h-screen bg-dark-bg-primary">
        ...
        <main className="relative z-10 pt-16">
          ...
          {/* Search */}
          <section className="py-12 bg-dark-bg-secondary border-t border-makr-blue/30">
            <div className="max-w-2xl mx-auto px-6">
              <div className="relative">
                <Search className="w-5 h-5 absolute left-4 top-1/2 transform -translate-y-1/2 text-dark-text-muted" />
                <input
                  type="text"
                  placeholder="Search documentation..."
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-12 pr-4 py-4 text-lg bg-dark-bg-primary border border-makr-blue/30 rounded-2xl text-dark-text-primary placeholder-dark-text-muted focus:ring-2 focus:ring-makr-yellow/50 focus:border-makr-yellow transition-all shadow-neon font-mono"
                />
              </div>
            </div>
          </section>
                ...
                <div className="space-y-4">
                  {filteredItems.map((item, index) => (
                    <Link
                      key={index}
                      to={item.href}
                      className="block card-cyberpunk p-6 hover:shadow-neon-lg transition-all group"
                    >
                        ...
                        <div className="space-y-3">
                          {section.items.map((item, index) => (
                            <Link
                              key={index}
                              to={item.href}
                              className="block p-4 rounded-lg border border-makr-blue/20 hover:border-makr-blue/40 hover:bg-makr-blue/5 transition-all group"
                            >
          ...
          <section className="py-20 bg-gradient-to-r from-makr-blue/20 to-purple-600/20 border-t border-makr-blue/30">
            <div className="max-w-4xl mx-auto px-6 text-center">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                <Link
                  to="/contact"
                  className="btn-cyberpunk px-6 py-3 rounded-lg font-semibold transition-all hover:scale-105 font-mono inline-flex items-center justify-center gap-2"
                >
